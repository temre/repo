...
public class StockEntry extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Do get of Stock entry");
		...
		if(loginEn.getStatus().equals(AppConstant.LoggedIn))
		{
			...
			RequestDispatcher rd= request.getRequestDispatcher("/WEB-INF/jsp/StockEntry.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Do post of stock entry");
		StockEEn stockEn;
		boolean result=false;
		try {
			stockEn = new SetEntityObject().setStockEEn(request);
			...
			// for admin : admin will add data of a particular user
						...
						if(null != userType && userType.equals("admin"))
						{
							...
							if(isUserExist)
							{
								stockEn.setUserId(user);
								result = new StockEntryService().persistSalesEn(stockEn);
							}
							else
							{
